...
def compose_answer(action, code, response, text, from_client=None):
    if action in ['authenticate', 'presence', 'msg', 'quit', 'get_contacts', 'add_contact', 'authentication'] and code == 'alert':
        answer_server = {
            'response': response,
            'alert': text,
    ...
    if action in ['authenticate', 'presence', 'msg', 'add_contact', 'authentication'] and code == 'error':
        answer_server = {
            'response': response,
            'error': text,
            "time": time.ctime(time.time()) }
