...
class ClientTransport(threading.Thread, QObject):
    ...
    def __init__(self, port, ip_address, database, username, encoding, passwd, keys):
    ...
    def connection_init(self, port, ip):
        ...
        if not connected:
            ...
            with socket_lock:
                ...
                self.transport.send(json.dumps(presense).encode(self.encoding))
                data = self.transport.recv(1024)
                data = json.loads(data.decode(self.encoding))
                if data['response'] == 400:
                    self.messages.critical(self, data['error'])
                if data['response'] == 200:
                    self.messages.critical(self, data['alert'])
