...
from destop_client.client_database import ClientDatabase
...
class ClientMainWindow(QMainWindow):
    def __init__(self, database, transport):
        ...
        self.name_client = self.transport.username
    ...
    def history_list_update(self):
        ...
        list_messages = sorted(self.database.list_history_messege(self.current_chat), key= lambda item: item[0])
        print(list_messages)
        ...
        for i in list_messages:
            if i[1] == self.current_chat:
                i = str(i[1])
                mess = QStandardItem(f'Входящее от {i[:3]}')
                mess.setEditable(False)
                mess.setBackground(QBrush(QColor(255, 213, 213)))
                mess.setTextAlignment(Qt.AlignLeft)
                self.history_model.appendRow(mess)
            if i[2] == self.current_chat:
                i = str(i[2])
                mess = QStandardItem(f'Исходящее от {i[:3]}')
                mess.setEditable(False)
                mess.setBackground(QBrush(QColor(255, 213, 213)))
                mess.setTextAlignment(Qt.AlignLeft)
                self.history_model.appendRow(mess)
    ...
    def clients_list_update(self):
        ...
        contacts_list = self.database.print_list_contact()
        self.contacts_model = QStandardItemModel()
        for i in sorted(contacts_list):
            item = QStandardItem(i.name_contact_client)
    ...
    def add_contact(self, new_contact):
        ...
        try:
            ...
            if data['response'] == 403:
                text = data['alert']
                self.messages.information(self, 'Ошибка', text)
    ...
    def delete_contact(self, item):
        ...
        try:
            ...
            if data['response'] == 202:
                ...
                self.clients_list_update()
                text = data['alert']
                self.messages.information(self, 'Успех', text)
                item.close()
