                            ...
                            if data['action'] == 'authentication':
                                for list_write_client in list_write:
                                    if list_reader_client == list_write_client:
                                        ...
                                        if search and search.password != key:
                                            text = 'Вы неправильно ввели пароль!!'
                                            client.send(
                                                json.dumps(compose_answer('authentication', 'error', 400, text)).encode(
                                                    self.encoding))
